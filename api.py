...
def get_network_detail(net_id: int):
    ...
    ofac_matches = []
    for m in members:
        if m in GLOBAL_STATE['flagged'] and GLOBAL_STATE['flagged'][m].ofac_match:
            ofac_matches.append(m)
            
    # Synthetic breakdowns since we don't have exact signal fields in `nets`
    # Just returning a placeholder based on score
    return {
        "net_id": net_id,
        "score": score,
        "risk": nrisk,
        ...
        "first_seen": "Unknown", # Requires iterating tx to find first seen
...
def get_globe_points():
    ...
    for addr, fa in flagged.items():
        # Using pseudo coordinates from pattern_detector
...
def get_globe_edges(net_id: int):
    ...
    inf = nets[net_id]
    members = sorted(m for m in inf["members"] if isinstance(m, str))[:40]
